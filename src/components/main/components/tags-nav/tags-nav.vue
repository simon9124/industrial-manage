...
export default {
  ...
  mounted () {
    ...
    // 事件总线：删除菜单时，删除相应的tag
    this.$EventBus.$on("tagDelete", tag => {
      const route = {
        meta: {
          icon: tag.ico,
          id: tag.id,
    ...
    // 事件总线：更新菜单时，更新相应的tag
    this.$EventBus.$on("tagUpdate", tagObj => {
      this.list.forEach((tag, i) => {
        if (tag.name === tagObj.dataOrg.name) {
          const tagUpdate = {
            meta: {
              icon: tagObj.dataUpdate.ico,
              id: tagObj.dataUpdate.id,
              title: tagObj.dataUpdate.title
            },
            name: tagObj.dataUpdate.name,
            params: {},
            query: {}
          };
          this.$set(this.list, i, tagUpdate);
          this.$emit("on-close", this.list, undefined, tagUpdate);
        }
      });
    });
  }
